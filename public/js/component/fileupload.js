...
define(function(require, exports, module) {
    ...
    // );
    $("body").append("<script type='text/vbscript'>\r\n" + "Function IEBinary_getByteAt(strBinary, iOffset)\r\n" + "    IEBinary_getByteAt = AscB(MidB(strBinary, iOffset + 1, 1))\r\n" + "End Function\r\n" + "Function IEBinary_getBytesAt(strBinary, iOffset, iLength)\r\n" + "  Dim aBytes()\r\n" + "  ReDim aBytes(iLength - 1)\r\n" + "  For i = 0 To iLength - 1\r\n" + "   aBytes(i) = IEBinary_getByteAt(strBinary, iOffset + i)\r\n" + "  Next\r\n" + "  IEBinary_getBytesAt = aBytes\r\n" + "End Function\r\n" + "Function IEBinary_getLength(strBinary)\r\n" + "    IEBinary_getLength = LenB(strBinary)\r\n" + "End Function\r\n" + "</script>\r\n");
    ...
    (function($) {
        var doc = document,
            slice = [].slice,
            defOpts = {
                progress: false, // false：关闭，true: 开启
                showArea: "",
                isPush: true,
                serverUrl: ""
            };
        ...
        function Upload(el, opts) {
            this.$el = $(el);
            if (this.$el.length < 1) return null;
        ...
        Upload.prototype = {
            ...
            _setOptions: function(opts) {
                this.opts = opts;
                this.$showArea = $(opts.showArea);
                this.progress = opts.progress;
                this.serverUrl = opts.serverUrl;
                this.state = false; // true表示正在上传
            },
            _init: function(opts) {
                var that = this;
                $.ajaxSettings.xhr = function() {
                    ...
                    if (that.progress) {
                        xhr.upload.addEventListener("progress", $.proxy(that._updProgressHandler, that));
                    }
                    return xhr;
                };
                that._bindEvent();
            ...
            _trigger: function() {
                var args = slice.call(arguments, 0),
                    eve = args.shift(0);
                if (!eve) return;
                this.$el.trigger(eve, args);
            },
            _bindEvent: function() {
                var that = this;
                that.$el.on("change", function(e) {
                    if (that.state) {
                        alert("请等待上一张图片上传完成");
                        return;
                    }
                    ...
                    canvasResize(inputFile, {
                        ...
                        callback: function(data, width, height) {
                            !that.opts.isPush && that.$showArea.html("");
                            var $item = $('<div class="item"></div>');
                            $item.appendTo(that.$showArea);
                            that.state = true;
                            that._trigger(EveProcess.Bef);
                            ...
                            if ($.os.android) {
                                ...
                                reader.onload = function() {
                                    that._uploadData(this.result, $item, mimetype);
                                };
                                reader.readAsArrayBuffer(file);
                            } else {
                                var form = new FormData();
                                form.append("file", file);
                                that._uploadData(form, $item, mimetype);
                            }
                        }
                    })
                });
            },
            _uploadData: function(data, $placeholder, mimetype) {
                var that = this;
                var serverUrl = that.serverUrl;
                $.ajax({
                    type: "POST",
                    url: serverUrl + "?mimetype=" + mimetype,
                    data: data,
                    dataType: "json",
                    /* 设置为false，阻止自动添加content－type头，否则contenttype中会丢失boundary值（jquery/zepto会自动设置为默认的表单提交类型）
                     * Content-Type=multipart/form-data; boundary=---------------------------99614912995 
                     * 
                     * 因为上传文件不在使用原有的http协议了。请求内容不再可能以 x = y方式发送了。而使用了分隔符boundary，因此丢失了分隔符就会发送失败
                     * 分隔符 
                     * 字段内容 
                     * 分隔符号 
                     * 字段内容2 
                     * 
                     * 设置为false，使用FormData方式时，值为multipart/form-data；安卓下使用的是二进制流，请求头里面没有这个项
                     */
                    contentType: false,
                    // 设置为false，否则jquery会自动将data转换为字符串
                    ...
                    success: function(resp) {
                        var imgurl = resp.thumbUrl;
                        var temp = '<img src="' + imgurl + '">';
                        $placeholder.append(temp);
                        that.$el.val("");
                        that._trigger(EveProcess.Sus, imgurl);
                    },
                    error: function() {
                        alert('服务器故障，上传失败');
                        that._trigger(EveProcess.Err);
                    },
                    complete: function() {
                        that.state = false;
                        that._trigger(EveProcess.End);
                    }
                })
            },
            _updProgressHandler: function(e) {
                if (e.lengthComputable) {
                    var precent = Math.ceil((e.loaded / e.total) * 100);
                    this._trigger(EveProcess.Ing, precent);
                }
            }
        };
        ...
        $.fn.upload = function(opts) {
            ...
            if ($(opts.showArea).length < 1) {
                var $div = $('<div class="showArea">');
                $(doc.body).append($div);
                opts.showArea = $div;
            }
            if (this.uploadObj) {
                delete this.uploadObj;
                this.uploadObj = null;
            }
            this.uploadObj = new Upload(this, opts)
            return this;
        };
        ...
        // });
        // $("").on("success:upload", function (e, precent) {
